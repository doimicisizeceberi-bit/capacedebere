...
export async function GET(req: Request) {
  try {
    ...
    // NEW: tag filters (optional)
    const tagIdsRaw = String(url.searchParams.get("tag_ids") || "").trim();
    const tagModeRaw = String(url.searchParams.get("tag_mode") || "or")
      .trim()
      .toLowerCase();
    const tagMode: "or" | "and" = tagModeRaw === "and" ? "and" : "or";
